...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    @if($featured->count())
    <div class="mb-14 relative group" id="hero-carousel">
        <div class="overflow-hidden rounded-4xl shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-200/50 dark:border-slate-800">
                ...
                <div class="min-w-full relative h-112.5 sm:h-137.5">
                    ...
                    <div class="absolute inset-0 flex flex-col justify-end p-8 sm:p-14">
                        ...
                        <a href="{{ route('article.show', $article->id) }}" class="block w-fit">
                            <h2 class="text-white text-3xl sm:text-5xl font-black tracking-tight leading-tight mb-4 hover:text-orange-400 transition-colors duration-300 line-clamp-3">
                                {{ $article->title }}
                            </h2>
                        </a>
                        <p class="text-slate-300 text-sm sm:text-base mb-6 line-clamp-2 max-w-3xl font-medium leading-relaxed">{{ $article->excerpt }}</p>
                        <div class="flex items-center gap-4 text-slate-300 text-sm font-semibold">
                            <span class="flex items-center gap-1.5"><i data-lucide="pen-line" class="w-4 h-4 text-orange-400"></i> {{ $article->author }}</span>
                            <span class="w-1 h-1 rounded-full bg-slate-500"></span>
                            <span class="flex items-center gap-1.5"><i data-lucide="clock-3" class="w-4 h-4 text-purple-400"></i> {{ $article->read_time }}</span>
                            @if($article->rating) 
                            <span class="w-1 h-1 rounded-full bg-slate-500"></span>
                            <span class="flex items-center gap-1.5 text-amber-400"><i data-lucide="star" class="w-4 h-4 fill-amber-400"></i> {{ $article->rating }}</span> 
                            @endif
                        </div>
                    </div>
                </div>
        ...
        {{-- Custom Dots --}}
        <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-slate-900/50 backdrop-blur-md px-4 py-2 rounded-full border border-white/10" id="carousel-dots">
            @foreach($featured as $i => $_)
            <button onclick="goToSlide({{ $i }})" class="carousel-dot w-2 h-2 rounded-full transition-all duration-300 {{ $i === 0 ? 'bg-orange-500 w-8' : 'bg-white/50 hover:bg-white' }}"></button>
            @endforeach
        </div>
        {{-- Navigation --}}
        <button onclick="prevSlide()" class="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white rounded-full flex items-center justify-center transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110"><i data-lucide="arrow-left" class="w-5 h-5"></i></button>
        <button onclick="nextSlide()" class="absolute right-6 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white rounded-full flex items-center justify-center transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110"><i data-lucide="arrow-right" class="w-5 h-5"></i></button>
    </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <section class="group/section">
                <div class="flex items-center justify-between mb-8 pb-4 border-b border-slate-200 dark:border-slate-800">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-2xl font-black tracking-tight text-slate-900 dark:text-white capitalize">{{ $meta['label'] }}</h2>
                    </div>
                    <a href="{{ route('category.show', $cat) }}" class="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-orange-500 transition-colors">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="h-full">
                        <a href="{{ route('article.show', $main->id) }}" class="group block h-full bg-white dark:bg-slate-900 rounded-3xl p-3 border border-slate-200 dark:border-slate-800 hover:shadow-2xl hover:shadow-orange-500/5 hover:-translate-y-1 transition-all duration-300">
                            ...
                            <div class="px-3 pb-3">
                                <h3 class="text-xl font-bold text-slate-900 dark:text-white leading-snug group-hover:text-orange-500 transition-colors line-clamp-3 mb-3">
                                    {{ $main->title }}
                                </h3>
                                <div class="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-xs font-semibold">
                                    <i data-lucide="pen-line" class="w-3.5 h-3.5"></i> <span>{{ $main->author }}</span>
                                </div>
                            </div>
                        </a>
                    </div>
                        ...
                        <a href="{{ route('article.show', $art->id) }}" class="group flex gap-4 p-3 rounded-2xl bg-slate-50 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-800 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 hover:shadow-xl hover:shadow-slate-200/20 dark:hover:shadow-none transition-all duration-300">
                            ...
                            <div class="flex flex-col justify-center min-w-0 py-1">
                                ...
                                <div class="flex items-center gap-3 text-slate-400 text-xs font-medium">
                                    <span class="flex items-center gap-1"><i data-lucide="clock-3" class="w-3.5 h-3.5"></i> {{ $art->read_time }}</span>
                                    @if($art->rating)
                                    <span class="flex items-center gap-1 text-amber-500"><i data-lucide="star" class="w-3.5 h-3.5 fill-amber-500"></i> {{ $art->rating }}</span>
                                    @endif
                                </div>
                            </div>
                        </a>
        ...
        <div class="space-y-8 lg:sticky lg:top-24 h-fit">
            ...
            {{-- Search Quick Widget --}}
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-2">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <span class="text-slate-900 dark:text-white font-black tracking-tight text-lg">Terbaru</span>
                </div>
                <div class="space-y-4">
                    @foreach($latest as $i => $art)
                    <a href="{{ route('article.show', $art->id) }}" class="group flex gap-4 items-start">
                        <span class="shrink-0 text-3xl font-black tabular-nums tracking-tighter transition-colors duration-300 {{ $i === 0 ? 'text-orange-500' : 'text-slate-200 dark:text-slate-800 group-hover:text-slate-400' }}">
                            {{ str_pad($i+1, 2, '0', STR_PAD_LEFT) }}
                        </span>
                        <div class="min-w-0 pt-1">
                            <h3 class="text-slate-800 dark:text-slate-200 text-sm font-bold leading-snug group-hover:text-orange-500 transition-colors line-clamp-2 mb-1.5">
                                {{ $art->title }}
                            </h3>
