...
<aside id="sidebar" class="fixed lg:static inset-y-0 left-0 z-50 w-72 bg-slate-950 border-r border-slate-800 flex flex-col shrink-0 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-out shadow-2xl lg:shadow-none">
    ...
    <nav class="flex-1 p-3 md:p-4 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('pewarta.dashboard') }}" class="touch-safe group w-full flex items-center gap-3 px-3.5 md:px-4 py-2.5 md:py-3 rounded-xl text-sm transition-all duration-300 ease-out bg-gradient-to-r from-sky-500 to-sky-600 text-white shadow-md shadow-sky-500/20 font-bold">
            <i data-lucide="layout-dashboard" class="w-4 h-4 md:w-5 md:h-5 group-hover:scale-110 transition-transform"></i>
            Dashboard
        </a>
        <a href="{{ route('pewarta.articles.create') }}" class="touch-safe group w-full flex items-center gap-3 px-3.5 md:px-4 py-2.5 md:py-3 rounded-xl text-sm transition-all duration-300 ease-out text-slate-400 hover:text-white hover:bg-slate-800/50 font-medium border border-transparent hover:border-slate-700">
            <i data-lucide="pen-line" class="w-4 h-4 md:w-5 md:h-5 group-hover:scale-110 transition-transform"></i>
            Tulis Artikel
        </a>
    </nav>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl md:rounded-2xl p-3 md:p-4 shadow-sm">
            <div class="flex items-center gap-2.5 md:gap-3 mb-3 md:mb-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-slate-500 text-[10px] md:text-xs truncate">Pewarta Zverse</p>
                </div>
            </div>
...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50">
    ...
    <header class="bg-white border-b border-slate-200 px-4 md:px-6 py-2.5 md:py-3.5 flex items-center justify-between shrink-0 z-10 shadow-sm shadow-slate-200/20">
        <div class="flex items-center gap-2.5 md:gap-4">
            ...
            <div>
                <h1 class="text-base sm:text-lg md:text-2xl font-black text-slate-900 tracking-tight leading-none mb-0.5 md:mb-1">Dashboard Pewarta</h1>
                <p class="text-[11px] sm:text-xs md:text-sm text-slate-500 font-medium">Selamat datang kembali, {{ explode(' ', $user->display_name)[0] }}</p>
            </div>
        </div>
        
        <a href="{{ route('pewarta.articles.create') }}" class="touch-safe hidden sm:flex items-center gap-1.5 md:gap-2 bg-sky-500 hover:bg-sky-600 text-white text-xs md:text-sm font-bold px-3 md:px-5 py-2 md:py-2.5 rounded-lg md:rounded-xl transition-all duration-300 shadow-sm hover:shadow-sky-500/25 hover:-translate-y-0.5">
            <i data-lucide="plus" class="w-3.5 h-3.5 md:w-4 md:h-4"></i> Tulis Artikel
        </a>
    </header>
    ...
    <main class="flex-1 overflow-y-auto p-3 md:p-6 lg:p-8">
        <div class="max-w-6xl mx-auto">
            ...
            {{-- Articles Table --}}
            <div class="bg-white rounded-2xl md:rounded-[1.5rem] border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-4 md:px-6 py-4 md:py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <h2 class="font-bold text-slate-900 flex items-center gap-1.5 md:gap-2">
                        <i data-lucide="file-text" class="w-4 h-4 md:w-5 md:h-5 text-sky-500"></i> <span class="text-sm md:text-base">Artikel Saya</span>
                        <span class="bg-slate-200 text-slate-600 text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 rounded-full">{{ $articles->count() }}</span>
                    </h2>
                    <a href="{{ route('pewarta.articles.create') }}" class="sm:hidden text-sky-600 hover:text-sky-700 font-bold text-xs md:text-sm flex items-center gap-1">
                        <i data-lucide="plus" class="w-3.5 h-3.5 md:w-4 md:h-4"></i> Tulis
                    </a>
                </div>
                ...
                <div class="divide-y divide-slate-100">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4 px-4 md:px-6 py-4 md:py-5 hover:bg-slate-50/80 transition-colors group">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-1.5 md:gap-2 mb-1.5 md:mb-2 flex-wrap">
                                <span class="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider px-2 md:px-2.5 py-0.5 md:py-1 rounded-full border font-bold shadow-sm {{ $badgeClass }}">
                                    <i data-lucide="{{ $statusIcons[$article->status] ?? 'circle' }}" class="w-2.5 h-2.5 md:w-3 md:h-3"></i> {{ $label }}
                                </span>
                                @if($article->review_note && in_array($article->status, ['rejected','approved']))
                                <span class="flex items-center gap-1 text-[10px] md:text-xs text-rose-500 bg-rose-50 px-2 md:px-2.5 py-0.5 md:py-1 rounded-lg border border-rose-100 font-medium">
                                    <i data-lucide="message-square-dashed" class="w-3 h-3 md:w-3.5 md:h-3.5"></i> Catatan
                                </span>
                                @endif
                            </div>
                            
                            <p class="text-slate-900 font-bold text-sm md:text-base leading-snug line-clamp-2 mb-1 md:mb-1.5 group-hover:text-sky-600 transition-colors">{{ $article->title }}</p>
                            
                            @if($article->review_note && in_array($article->status, ['rejected','approved']))
                            <p class="text-xs md:text-sm text-slate-500 italic border-l-2 border-rose-200 pl-3 mb-1.5 md:mb-2 bg-slate-50 py-1 pr-2 rounded-r-lg line-clamp-2">"{{ $article->review_note }}"</p>
                            @endif

                            <div class="flex items-center gap-2 md:gap-3 text-slate-400 text-[11px] md:text-xs font-semibold">
                                <span class="flex items-center gap-1"><i data-lucide="layout-grid" class="w-3 h-3 md:w-3.5 md:h-3.5"></i> {{ ucfirst($article->category) }}</span>
                                <span class="text-slate-300">•</span>
                                <span class="flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3 md:w-3.5 md:h-3.5"></i> {{ $article->created_at->diffForHumans() }}</span>
                                @if($article->submitted_at) 
                                <span class="text-slate-300">•</span>
                                <span class="flex items-center gap-1"><i data-lucide="calendar-check" class="w-3 h-3 md:w-3.5 md:h-3.5"></i> {{ $article->submitted_at->format('d M Y') }}</span> 
                                @endif
                            </div>
                        </div>
                        
                        <div class="flex flex-wrap items-center gap-1.5 md:gap-2 shrink-0 border-t md:border-t-0 pt-2.5 md:pt-0 mt-2 md:mt-0 border-slate-100">
                            @if(in_array($article->status, ['draft','rejected']))
                                <a href="{{ route('pewarta.articles.edit', $article->id) }}" class="touch-safe flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs bg-white hover:bg-sky-50 text-slate-600 hover:text-sky-600 border border-slate-200 hover:border-sky-200 px-2.5 md:px-3 py-1.5 md:py-2 rounded-lg md:rounded-xl transition-all shadow-sm font-bold">
                                    <i data-lucide="edit-3" class="w-3 h-3 md:w-3.5 md:h-3.5"></i> Edit
                                </a>
                            @endif
                            ...
                            @if($article->status === 'draft')
                                <form action="{{ route('pewarta.articles.submit', $article->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" onclick="return confirm('Kirim artikel ini ke redaksi untuk direview?')" class="touch-safe flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs bg-sky-500 hover:bg-sky-600 text-white px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl transition-all shadow-sm shadow-sky-500/20 font-bold">
                                        <i data-lucide="send" class="w-3 h-3 md:w-3.5 md:h-3.5"></i> Submit
                                    </button>
                                </form>
                            @endif
                            ...
                            @if(in_array($article->status, ['draft','rejected']))
                                <form action="{{ route('pewarta.articles.destroy', $article->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus artikel ini secara permanen?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="touch-safe flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs bg-white hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 hover:border-rose-200 px-2.5 md:px-3 py-1.5 md:py-2 rounded-lg md:rounded-xl transition-all shadow-sm font-bold">
                            ...
                            @if($article->status === 'published')
                                <a href="{{ route('article.show', $article->slug) }}" target="_blank" class="touch-safe flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl font-bold hover:bg-emerald-100 transition-colors">
                                    <i data-lucide="external-link" class="w-3 h-3 md:w-3.5 md:h-3.5"></i> Lihat Web
                                </a>
                            @endif
                        </div>
                    </div>
